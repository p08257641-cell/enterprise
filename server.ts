...
app.get('/api/bootstrap', asyncHandler(async (req, res) => {
  ...
  const [
    ...
    wData, kData, logData, posProdData, posCustData, posSalesData, posCatData,
    posTermData, posShiftData, posDiscData, posRetData, posReportData, leavesData,
    attData, okrsData, slipsData, jeData, expData, fpData, obData, billData,
    bpPayData, cpData, baData, btxData, brData, faData, deData, budData, ccData,
    onbData, pgData, sbData, soData, scData, sqData, stData, kbData, lmsData,
    annData, wtData, etData, chatData, chatGroupsData, pollsData, pollOptsData,
    pollVotesData, imgData, rolesData, policiesData, approvalsData
  ] = await Promise.all([
    dbAll(schema.companies),
    dbAll(schema.users),
    dbAll(schema.employees),
    dbAll(schema.applicants),
    dbAll(schema.departments),
    dbAll(schema.branches),
    dbAll(schema.crmLeads),
    dbAll(schema.glAccounts),
    dbAll(schema.inventory),
    dbAll(schema.tickets),
    dbAll(schema.workflows),
    dbAll(schema.apiKeys),
    dbAll(schema.auditLogs),
    dbAll(schema.posProducts),
    dbAll(schema.posCustomers),
    dbAll(schema.posSales),
    dbAll(schema.posCategories),
    dbAll(schema.posTerminals),
    dbAll(schema.posShifts),
    dbAll(schema.posDiscounts),
    dbAll(schema.posReturns),
    dbAll(schema.posDailyReports),
    dbAll(schema.leaves),
    dbAll(schema.attendance),
    dbAll(schema.okrs),
    dbAll(schema.payslips),
    dbAll(schema.journalEntries),
    dbAll(schema.expenses),
    dbAll(schema.fiscalPeriods),
    dbAll(schema.openingBalances),
    dbAll(schema.bills),
    dbAll(schema.billPayments),
    dbAll(schema.customerPayments),
    dbAll(schema.bankAccounts),
    dbAll(schema.bankTransactions),
    dbAll(schema.bankReconciliations),
    dbAll(schema.fixedAssets),
    dbAll(schema.depreciationEntries),
    dbAll(schema.budgets),
    dbAll(schema.costCenters),
    dbAll(schema.onboardings),
    dbAll(schema.payrollGroups),
    dbAll(schema.salaryBands),
    dbAll(schema.salesOrders),
    dbAll(schema.salesCustomers),
    dbAll(schema.salesQuotations),
    dbAll(schema.salesTargets),
    dbAll(schema.kbArticles),
    dbAll(schema.lmsCourses),
    dbAll(schema.announcements),
    dbAll(schema.workflowTriggers),
    dbAll(schema.emailTemplates),
    dbAll(schema.chatMessages),
    dbAll(schema.chatGroups),
    dbAll(schema.polls),
    dbAll(schema.pollOptions),
    dbAll(schema.pollVotes),
    dbAll(schema.companyImages),
    dbAll(customRoles),
    dbAll(approvalPolicies),
    dbAll(pendingApprovals)
  ]);

  const invoices = await dbAll(schema.invoices);
