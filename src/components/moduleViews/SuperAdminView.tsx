      ...
      if (selectedUser.activeRole !== 'Super Admin') {
        return (
          <div className="flex flex-col items-center justify-center py-24">
            <i className="bi bi-shield-exclamation fs-4xl text-rose-400 mb-4"></i>
            <h2 className="fs-lg fw-bold text-slate-900 mb-1">Access Restricted</h2>
            <p className="fs-sm text-slate-500">This console is reserved for Super Administrators only.</p>
          </div>
        );
      }
      return (
        <div>
          {maintenanceMode && (
            <div className="mb-5 flex items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 fs-xs fw-semibold text-rose-700">
          ...
          {platformMsg && (
            <div className={`mb-5 rounded-xl border px-4 py-3 fs-xs fw-semibold ${platformMsg.type === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-amber-200 bg-amber-50 text-amber-700'}`}>{platformMsg.text}</div>
          )}
          <div className="flex items-start justify-between pb-5 border-b border-slate-200 mb-6">
            <div>
              ...
              <h1 className="fs-xl fw-bold tracking-tight text-slate-900">Platform System Console</h1>
              <p className="fs-sm text-slate-500 mt-0.5">Global infrastructure controls, tenant management, billing operations and platform configuration.</p>
            </div>
          </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl shadow-xs p-5">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <button key={a.label} onClick={() => handlePlatformAction(a.label)} className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg border fs-xs fw-semibold cursor-pointer transition-all ${a.danger ? 'border-rose-200 text-rose-700 hover:bg-rose-50' : 'border-slate-200 text-slate-700 hover:bg-slate-50'}`}>
                      <i className={`${a.icon} fs-sm`}></i>{a.label}
                    </button>
                  ))}
                </div>
              </div>
          ...
          {showNoticeModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
              <div className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-6 shadow-2xl">
                <h2 className="fs-sm fw-semibold text-slate-900 uppercase tracking-wide border-b border-slate-100 pb-3 mb-4">Send Platform Notice</h2>
                <div className="space-y-4">
                  ...
                  <div><Label>Message</Label><textarea value={noticeBody} onChange={e => setNoticeBody(e.target.value)} rows={4} placeholder="Message broadcast to all tenants…" className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 fs-xs text-slate-900 placeholder-slate-400 focus:border-slate-400 focus:outline-none resize-none" /></div>
                </div>
