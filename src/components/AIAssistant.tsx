    ...
    <div className="flex h-[calc(100vh-10rem)] flex-col rounded-xl border border-slate-200/80 bg-white shadow-xs overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch border-b border-slate-200 bg-white px-5 py-4 gap-3">
        ...
        <div className="flex gap-1.5 overflow-x-auto self-center">
          ...
          >
            <i className="bi bi-chat-left-text text-xs"></i>
            General Chat
          </button>
          <button
          ...
          >
            <i className="bi bi-lightbulb text-xs"></i>
            AI Smart Insights
          </button>
          <button
      ...
      {activeTab === 'insights' ? (
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50 space-y-6">
          <div className="flex justify-between items-center pb-3 border-b border-slate-200">
            <div>
              <h3 className="text-sm font-bold text-slate-950">AI Smart Trend Insights</h3>
            ...
            <button
              ...
              disabled={scanning}
              className="bg-slate-950 hover:bg-slate-800 disabled:bg-slate-200 text-white font-semibold text-[10px] px-3.5 py-1.5 rounded-lg transition-all cursor-pointer shadow-2xs"
            >
              {scanning ? (
                <>
          ...
          {scanning ? (
            <div className="h-64 flex flex-col items-center justify-center space-y-3">
              <i className="bi bi-cpu-fill text-3xl text-slate-900 animate-spin"></i>
              <p className="text-xs text-slate-500 font-medium">Analyzing active ledgers, inventory reorders, work orders completion rates, and HR timesheets...</p>
            </div>
          ) : (
                ...
                <div key={insight.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-3xs flex flex-col justify-between hover:shadow-xs transition-all">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-[9px] text-slate-400 font-mono font-medium">Severity: {insight.severity}</span>
                    </div>
                    <p className="text-xs font-semibold text-slate-900 leading-snug">{insight.desc}</p>
                  </div>
                  <div className="mt-4 pt-4 border-t border-slate-100 bg-slate-50/50 -mx-5 -mb-5 p-4 rounded-b-xl">
                    <span className="text-[9px] font-bold text-slate-400 uppercase block mb-1">Recommended Action</span>
